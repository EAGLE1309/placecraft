...
export async function POST(request: NextRequest) {
  try {
    ...
    const fileName = `resumes/${studentId}/${Date.now()}_${personalInfo.name.replace(/\s+/g, "_")}.pdf`;
    const uploadResult = await uploadToR2(pdfBuffer, fileName, "application/pdf");

    const improvementData: ResumeImprovementSnapshot = {
      personalInfo,
      education,
      experience,
      projects,
    ...
    const historyEntry = await createResumeHistory(studentId, {
    ...
    if (analyzeRes.ok) {
      ...
      console.log("Resume analyzed successfully:", analyzeData);
    }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to finalize resume" },
      { status: 500 }
    );
  }
}
